...
import { VIDEO_STUDENTS } from "@/lib/mock-videos"
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl bg-card p-4 shadow-md shadow-foreground/[0.04] sm:flex-row sm:items-center sm:flex-wrap">
      ...
      <div className="sm:w-48">
        <label htmlFor="video-person-select" className="sr-only">
          Сурагчийн нэр
        </label>
        <Select value={person} onValueChange={onPersonChange}>
            <SelectTrigger id="video-person-select" className="w-full rounded-xl border-transparent bg-secondary/50 text-foreground shadow-sm shadow-foreground/[0.03]">
      ...
      {/* Single date filter */}
      <div>
        <label htmlFor="video-date-filter" className="sr-only">
          Огноо
        </label>
        <Input
